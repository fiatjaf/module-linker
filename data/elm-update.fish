...
for pkg in (cat elm-all.json | jq -r 'keys | .[]')
  echo "$pkg"
  set vers (cat elm-all.json | jq -r --arg PKG "$pkg" '.[$PKG][-1]')
  echo "  version: $vers"
  ...
  echo "  desc: $desc"
  echo "  modules:"
  for moduleName in (curl --compressed -H "Content-Type: application/json" -H "Accept: application/json" -s "https://package.elm-lang.org/packages/$pkg/$vers/docs.json" | jq -rc '.[].name')
    echo "    $moduleName"
