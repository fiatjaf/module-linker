...
function processLine (elem, line, currentPath, lineIndex) {
  ...
  .then(() => {
    if (startswith(moduleName, '.')) {
      ...
      // is from the stdlib, so we're linking to the actual module files in github.com/crystal-lang/crystal
      return {
        url: `https://github.com/crystal-lang/crystal/blob/master/src/${moduleName}.cr`,
        kind: 'stdlib'
      }
    } else {
      ...
      return crystalurl(moduleName)
        .then(info => {
          if (info.url.match(/https?:\/\/github.com\/[^\/]+\/[^\/]+\/?$/)) {
            info.url = info.url.replace(/\/?$/, `/blob/master/src/${moduleName}.cr`)
          }
          return info
        })
    }
...
function crystalurl (moduleName) {
  return text(`https://raw.githubusercontent.com/fiatjaf/module-linker/backends/data/crystal-shards/${moduleName}`)
    .then(url => ({url: url, kind: 'external'}))
}
